<template>
  ...
  <div class="selectContainer__total">
    ...
    <div class="selectContainer__inactive" @click="Active()"
      :style="{ transition: $store.state.isActive ? '4s' : '4s', opacity: $store.state.isActive ? 0 : 1, display: $store.state.isActive ? 'none' : 'inline' }">
      <!-- 초기값 false -->
      ...
      <div class="shortInfo">
        <span>{{ $store.state.exMonth }}.{{ $store.state.exDate }}</span>~ <span>{{ $store.state.exMonth }}.{{
          $store.state.exDate
          }}</span>|
    ...
    <div class="selectContainer__active"
      :style="{ height: $store.state.isActive ? activeHeight : inactiveHeight, transition: $store.state.isActive ? '0.6s' : '1s', opacity: $store.state.isActive ? 1 : 0 }">
      <!-- 편도 - 왕복 토글 버튼 -->
      <ToggleButton/>
      ...
      <div class="flightContainer">
        ...
        <div>
          ...
          <Button layout='reverse' @click="reverse" color='base'
            :style="{ top: '4px', left: '1px', padding: '', display: 'relative' }">
            <div class="material-icons" :style="{ fontSize: '30px', display: 'flex', marginLeft: '-13px' }">
              compare_arrows
            </div>
          </Button>
        </div>
      ...
      <div class="set">
        ...
        <ul class="block">
          ...
          <li class="block-radio passengers">
            ...
            <label for="baby"><i class="material-icons">baby_changing_station</i>&times;&nbsp; <span> {{ person[2] }}
              </span>
              <h2> baby </h2>
      ...
      <div class="set">
        ...
        <Button @click="[searchTicket(), unActive()]" :style="{marginTop: '2.5em'}"
          :class="{ 'isnot-active': $store.state.isActive, [className]: false }"> Search </Button>
    </div>
  </div>
</template>
